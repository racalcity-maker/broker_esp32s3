...
".sequence-device{border:1px solid #1f2430;border-radius:10px;padding:10px;background:#0c0e12;}"
...
".sequence-pill{display:inline-flex;align-items:center;gap:6px;padding:5px 8px;border-radius:999px;background:#111827;border:1px solid #1f2937;color:#dbeafe;font-size:12px;}"
".sequence-details{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;}"
".sequence-detail{border:1px dashed #1f2430;border-radius:8px;padding:6px 8px;background:#05070b;color:#e5e7eb;font-size:12px;}"
".sequence-detail .label{display:block;color:#94a3b8;font-size:11px;margin-bottom:2px;}"
        ...
        ".track-card{border-radius:12px;width:100px;height:100px;display:flex;align-items:flex-end;justify-content:center;position:relative;overflow:hidden;color:#e2e8f0;border:1px solid rgba(255,255,255,0.08);background:linear-gradient(180deg,#1b2230 0%,#141b27 52%,#0f1520 100%);cursor:pointer;box-shadow:0 10px 22px rgba(2,6,23,0.34);z-index:1;transition:transform .15s ease,box-shadow .2s ease,border-color .2s ease,background .2s ease;}"
        ".track-card::after{content:'';position:absolute;inset:0;background:linear-gradient(180deg,rgba(255,255,255,0.08) 0%,rgba(255,255,255,0.02) 38%,rgba(15,23,42,0) 62%,rgba(2,6,23,0.42) 100%);pointer-events:none;}"
        ".track-card:hover{border-color:rgba(125,211,252,0.24);box-shadow:0 14px 26px rgba(2,6,23,0.42);transform:translateY(-1px);}"
        ".track-card.selected{border-color:#60a5fa;box-shadow:0 0 0 1px rgba(96,165,250,0.35),0 14px 26px rgba(2,6,23,0.44);}"
        ".track-card.playing{border-color:#7dd3fc;background:linear-gradient(180deg,#1f2a3c 0%,#162234 52%,#101925 100%);box-shadow:0 0 0 1px rgba(125,211,252,0.38),0 16px 30px rgba(8,47,73,0.42);z-index:3;transform:translateY(-2px);}"
        ".track-name{position:relative;z-index:1;width:100%;text-align:center;padding:10px 6px 9px;box-sizing:border-box;color:#f8fafc;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px;letter-spacing:0.01em;background:linear-gradient(180deg,rgba(8,11,18,0) 0%,rgba(8,11,18,0.62) 42%,rgba(8,11,18,0.9) 100%);}"
...
"</div>"
        ...
        "<div class='row'><button onclick='navUp()'>Up</button><button onclick='loadFiles()'>Refresh files</button><span id='path_label' class='muted small'>" SD_STORAGE_ROOT_PATH "</span><span class='muted small'>Click track to select</span></div>"
...
"function applyRoleLayout(role){const body=document.body;if(body){body.dataset.role=role||'admin';}if(role!=='user'){if(document.body){document.body.classList.remove('role-user');}tabs.forEach(tab=>{tab.style.display='';});return;}document.body.classList.add('role-user');const banner=document.getElementById('banner');if(banner){banner.textContent='Scenario console';}tabs.forEach(tab=>{if(tab.dataset.tab!=='actions'){tab.style.display='none';tab.classList.remove('active');}else{tab.style.display='';tab.classList.add('active');}});panes.forEach(pane=>{if(pane.id==='pane-actions'){pane.classList.add('active');}else{pane.classList.remove('active');}});}"
...
"function finishScrub(){progressScrubbing=false;updateProgressDisplay();commitScrub();}"
        ...
        "refreshModeButtons();"
"sessionRolePromise.then(info=>{if(info&&info.role==='user'){return;}loadStatus().then(()=>{if(statusInterval)clearInterval(statusInterval);statusInterval=setInterval(()=>{const activePane=document.querySelector('.pane.active');if(!activePane)return;const activeId=activePane.id;if(activeId==='pane-audio'){fetchAudioStatus();return;}if(activeId==='pane-status'||activeId==='pane-actions'){loadStatus().catch(()=>{});}},3000);});});"
