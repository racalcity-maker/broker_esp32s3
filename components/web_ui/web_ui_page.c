...
"function finishScrub(){progressScrubbing=false;updateProgressDisplay();commitScrub();}"
        ...
        "refreshModeButtons();"
"sessionRolePromise.then(info=>{if(info&&info.role==='user'){return;}loadStatus().then(()=>{if(statusInterval)clearInterval(statusInterval);statusInterval=setInterval(()=>{const activePane=document.querySelector('.pane.active');if(!activePane)return;const activeId=activePane.id;if(activeId==='pane-status'||activeId==='pane-audio'){fetchAudioStatus();}},3000);});});"
