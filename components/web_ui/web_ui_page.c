...
"function renderSensors(list){ const grid=document.getElementById('sensor_cards'); const empty=document.getElementById('sensor_empty'); if(!grid||!empty)return; const role=(window.__WEB_SESSION&&window.__WEB_SESSION.role)||'admin'; const showAll=role!=='user'; const sensors=Array.isArray(list)?list.filter(s=>s&&(showAll||s.display!==false)):[]; if(!sensors.length){grid.innerHTML='';empty.style.display='block';return;} empty.style.display='none'; const fmtValue=sensor=>{if(sensor&&typeof sensor.value==='number'){const dec=(typeof sensor.decimals==='number'&&sensor.decimals>=0&&sensor.decimals<=6)?sensor.decimals:1;return Number(sensor.value).toFixed(dec);}return'n/a';}; const fmtDuration=ms=>{if(typeof ms!=='number'||ms<=0)return'0ms';if(ms>=1000){const s=ms/1000;return s>=10?`${Math.round(s)}s`:`${s.toFixed(1)}s`;}return `${Math.round(ms)}ms`;}; const statusClass=status=>{switch(status){case'warn':return'status-warn';case'alarm':return'status-alarm';case'ok':return'status-ok';default:return'status-unknown';}}; const describeFilters=(th,sensor)=>{if(!th)return'';const parts=[];if(typeof th.hysteresis==='number'&&th.hysteresis>0){const dec=(typeof sensor.decimals==='number'&&sensor.decimals>=0&&sensor.decimals<=6)?sensor.decimals:1;parts.push(`&Delta;${Number(th.hysteresis).toFixed(dec)}`);}if(typeof th.min_duration_ms==='number'&&th.min_duration_ms>0){parts.push(`min ${fmtDuration(th.min_duration_ms)}`);}if(typeof th.cooldown_ms==='number'&&th.cooldown_ms>0){parts.push(`cool ${fmtDuration(th.cooldown_ms)}`);}return parts.length?` <span class='muted small'>(${parts.join(' &middot; ')})</span>`:'';}; const describeThreshold=(label,th,sensor)=>{if(!th||!th.enabled)return'';const cmp=th.compare==='below_or_equal'?'&le;':'&ge;';const dec=(typeof sensor.decimals==='number'&&sensor.decimals>=0&&sensor.decimals<=6)?sensor.decimals:1;const val=typeof th.value==='number'?Number(th.value).toFixed(dec):'';const units=sensor.units?` ${escapeHtml(sensor.units)}`:'';const scenario=th.scenario?` <span class='muted'>&rarr; ${escapeHtml(th.scenario)}</span>`:'';return `<div class='sensor-thresholds-line'><span class='label'>${label}</span><span>${cmp} ${val}${units}${scenario}${describeFilters(th,sensor)}</span></div>`;}; const renderHistory=sensor=>{if(!Array.isArray(sensor.history)||!sensor.history.length)return'';const dec=(typeof sensor.decimals==='number'&&sensor.decimals>=0&&sensor.decimals<=6)?sensor.decimals:1;const samples=sensor.history.slice(-4).reverse();const chips=samples.map(sample=>{const value=typeof sample.value==='number'?sample.value.toFixed(dec):'n/a';const ts=typeof sample.timestamp_ms==='number'?Math.round(sample.timestamp_ms/1000):'~';return `<span class='history-chip ${statusClass(sample.status)}' title='t+${ts}s'>${value}${sensor.units?` ${escapeHtml(sensor.units)}`:''}</span>`;}).join('');return `<div class='sensor-history'>${chips}</div>`;}; sensors.sort((a,b)=>{const da=(a.device_name||a.device||'').toLowerCase();const db=(b.device_name||b.device||'').toLowerCase();if(da!==db)return da.localeCompare(db);const an=(a.name||a.channel_id||a.topic||'').toLowerCase();const bn=(b.name||b.channel_id||b.topic||'').toLowerCase();return an.localeCompare(bn);}); grid.innerHTML=sensors.map(sensor=>{const status=String(sensor.status||'unknown');const cls=statusClass(status);const units=sensor.units?`<span class='sensor-units'>${escapeHtml(sensor.units)}</span>`:'';const deviceLine=(sensor.device_name||sensor.device)?`<div class='muted small'>${escapeHtml(sensor.device_name||sensor.device||'')}</div>`:'';const topic=sensor.topic?`<div class='sensor-topic'>${escapeHtml(sensor.topic)}</div>`:'';const desc=sensor.description?`<div class='muted small'>${escapeHtml(sensor.description)}</div>`:'';const updated=typeof sensor.updated_ms==='number'?`t+${Math.round(sensor.updated_ms/1000)}s`:'n/a';const thresholds=[describeThreshold('Warn',sensor.warn,sensor),describeThreshold('Alarm',sensor.alarm,sensor)].filter(Boolean).join('');const history=renderHistory(sensor);const channelBadge=sensor.channel_id?`<span class='muted small'>${escapeHtml(sensor.channel_id)}</span>`:'';return `<div class='sensor-card ${cls}'><div class='sensor-header'><div><div class='sensor-name'>${escapeHtml(sensor.name||sensor.device||'Sensor')}</div>${deviceLine}${topic}${desc}</div><span class='sensor-status-pill ${cls}'>${status.toUpperCase()}</span></div><div class='sensor-value'>${fmtValue(sensor)}${units}</div><div class='sensor-meta'><span>Updated ${updated}</span>${channelBadge}</div>${thresholds?`<div class='sensor-thresholds'>${thresholds}</div>`:''}${history}</div>`;}).join(''); }"
