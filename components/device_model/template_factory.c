...
static void dm_model_str_copy(char *dst, size_t dst_len, const char *src)
{
    if (!dst || dst_len == 0) {
        return;
    }
                                    ...
                                    const char *src_payload)
{
    if (src_topic && src_topic[0]) {
        dm_model_str_copy(topic, topic_len, src_topic);
        if (src_payload) {
            dm_model_str_copy(payload, payload_len, src_payload);
        } else {
            payload[0] = '\0';
        }
    } else {
...
esp_err_t dm_template_factory_build_uid(const dm_uid_template_params_t *params, dm_template_config_t *out)
{
    ...
    dm_model_str_copy(out->data.uid.success_audio_track,
                sizeof(out->data.uid.success_audio_track),
                params->success_audio_track);
    dm_model_str_copy(out->data.uid.fail_audio_track,
                sizeof(out->data.uid.fail_audio_track),
                params->fail_audio_track);
    dm_model_str_copy(out->data.uid.bg_track,
                sizeof(out->data.uid.bg_track),
                params->bg_track);
    dm_model_str_copy(out->data.uid.bg_start_topic,
...
esp_err_t dm_template_factory_build_signal_hold(const dm_signal_hold_params_t *params, dm_template_config_t *out)
{
    ...
    dm_model_str_copy(out->data.signal.signal_topic, sizeof(out->data.signal.signal_topic), params->signal_topic);
    dm_model_str_copy(out->data.signal.signal_payload_on,
                sizeof(out->data.signal.signal_payload_on),
                params->signal_payload_on);
    dm_model_str_copy(out->data.signal.signal_payload_off,
                sizeof(out->data.signal.signal_payload_off),
                params->signal_payload_off);
    out->data.signal.signal_on_ms = params->signal_on_ms;

    dm_model_str_copy(out->data.signal.heartbeat_topic, sizeof(out->data.signal.heartbeat_topic), params->heartbeat_topic);
    if (params->reset_topic) {
        dm_model_str_copy(out->data.signal.reset_topic, sizeof(out->data.signal.reset_topic), params->reset_topic);
    }
    out->data.signal.required_hold_ms = params->required_hold_ms;
    out->data.signal.heartbeat_timeout_ms = params->heartbeat_timeout_ms;

    dm_model_str_copy(out->data.signal.hold_track, sizeof(out->data.signal.hold_track), params->hold_track);
    out->data.signal.hold_track_loop = params->hold_track_loop;
    dm_model_str_copy(out->data.signal.complete_track, sizeof(out->data.signal.complete_track), params->complete_track);
