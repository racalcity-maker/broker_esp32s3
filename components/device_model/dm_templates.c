...
bool dm_uid_template_set_slot(dm_uid_template_t *tpl, uint8_t index, const char *source_id, const char *label)
{
    ...
    dm_model_str_copy(slot->source_id, sizeof(slot->source_id), source_id);
    if (label && label[0]) {
        dm_model_str_copy(slot->label, sizeof(slot->label), label);
    }
...
bool dm_uid_template_add_value(dm_uid_template_t *tpl, uint8_t slot_index, const char *value)
{
    ...
    dm_model_str_copy(slot->values[slot->value_count], sizeof(slot->values[slot->value_count]), value);
    slot->value_count++;
    return true;
}
