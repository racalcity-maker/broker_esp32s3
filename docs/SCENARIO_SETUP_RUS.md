...
**Цель:** принимать значения датчиков по MQTT, показывать их на вкладке Monitoring и запускать сценарии при достижении порогов предупреждения/аварии. В одном устройстве теперь можно описать **несколько каналов** (температура, влажность, CO₂ и т. п.), чтобы не размножать одинаковые шаблоны.

### Поля устройства
1. Добавьте устройство `sensor_room_a`, выберите шаблон **Sensor Monitor**.
2. Заполните:
   - `Base topic` — общий MQTT-топик (`room/a/telemetry`). Его указывают один раз; отдельный канал может переопределить свой топик.
   - `Parse mode` — **Raw number** для простых чисел или **JSON number** + `JSON key`, если payload — JSON (`{"temperature":23.5,"humidity":40}`).
   - `Display on Monitoring tab` / `History` — значения по умолчанию для новых каналов (видимость карточки и длина истории).
   - В секции **Channels** добавьте по одному каналу на каждый датчик:
     - Канал 1 (температура): `Channel ID = temp`, `Name = Temperature`, `JSON key = temperature`, `Units = °C`, `Decimals = 1`. Топик оставьте пустым, чтобы использовать базовый.
     - Канал 2 (влажность): `Channel ID = humidity`, `Name = Humidity`, `JSON key = humidity`, `Units = %`, `Decimals = 1`.
     - Канал 3 (CO2): `Channel ID = co2`, `Name = CO2`, `Topic = room/a/co2` (сырые числа без JSON).
   - Для каждого канала настройте `Warn` и `Alarm`: значение, знак сравнения и сценарий (`warn_temp`, `alarm_temp`, `warn_co2` и т. д.).
   - При необходимости включите фильтры устойчивости: `Hysteresis` (допуск), `Minimum duration (ms)` (минимальная длительность превышения) и `Cooldown (ms)` (задержка перед повторным запуском сценария).

### Сценарии
- `warn_temp`: отправить MQTT-подсказку (`room/a/hint -> "Температура растёт"`) и подсветить индикаторы.
- `alarm_temp`: включить сирену, отправить команду HVAC и выставить служебные флаги.
- `warn_co2`: запустить громкое сообщение или открыть клапан вентиляции.

### Проверка
Опубликуйте значения в соответствующие топики. Для JSON-каналов отправьте `{"temperature":27.5,"humidity":38}`, для CO₂ — `mosquitto_pub -t room/a/co2 -m 1200`. На вкладке Monitoring появятся отдельные карточки каналов с устройством, значением, порогами и историей. При пересечении порога статус и бейдж изменятся, а в логах появится строка вида `[Sensor] dev=sensor_room_a channel=temp status=warn`.
