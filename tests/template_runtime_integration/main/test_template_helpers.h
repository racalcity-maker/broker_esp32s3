...
static inline device_manager_config_t *template_test_alloc_config(uint8_t capacity)
{
    ...
    TEST_ASSERT_NOT_NULL_MESSAGE(cfg, "template runtime test config alloc failed");
    cfg->device_capacity = cap;
    return cfg;
}

static inline void template_test_free_config(device_manager_config_t *cfg)
{
    if (cfg) {
        heap_caps_free(cfg);
    }
...
static inline device_manager_config_t *template_test_parse_config_json(const char *json, uint8_t capacity)
{
    device_manager_config_t *cfg = template_test_alloc_config(capacity);
...
static inline const device_descriptor_t *template_test_find_device(const device_manager_config_t *cfg,
                                                                   const char *id)
{
    if (!cfg || !id) {
        return NULL;
...
static inline const device_descriptor_t *template_test_require_template(device_manager_config_t *cfg,
                                                                        const char *id,
                                                                        dm_template_type_t type)
{
    const device_descriptor_t *dev = template_test_find_device(cfg, id);
